const testRecipe = [
    {
    _id: '2134123;lkj',
    title: "Fried Chicken",
    subtitle: "Classic Southern Dish",
    ingredientsList: [
        '2lbs Chicken Breast',
        '32 oz Oil (Canola or Peanut)',
        '1/4 Cu Creole Seasoning',
        '3 Cu Flour',
        '1 Cu Buttermilk',
        '1 teaspoon paprika',
        'salt and pepper to taste'
    ],
    directions: [
        'Take your cut up chicken pieces and skin them if you prefer.',
        'Put the flour in a large plastic bag (let the amount of chicken you are cooking dictate the amount of flour you use). Season the flour with paprika, salt and pepper to taste (paprika helps to brown the chicken).',
        'Place the coated chicken on a cookie sheet or tray, and cover with a clean dish towel or waxed paper. LET SIT UNTIL THE FLOUR IS OF A PASTE-LIKE CONSISTENCY. THIS IS CRUCIAL!',
        'Fill a large skillet (cast iron is best) about 1/3 to 1/2 full with vegetable oil. Heat until VERY hot.',
        'Put in as many chicken pieces as the skillet can hold. Brown the chicken in HOT oil on both sides.',
        'When browned, reduce heat and cover skillet; let cook for 30 minutes (the chicken will be cooked through but not crispy). Remove cover, raise heat again, and continue to fry until crispy.',
        'Drain the fried chicken on paper towels. Depending on how much chicken you have, you may have to fry in a few shifts. Keep the finished chicken in a slightly warm oven while preparing the res',
    ],
    totalTime: '40 mins',
    prepTime: '15 mins'
},
{
    _id: 'asdf131241234lkj',
    title: "Fried Chicken",
    subtitle: "Classic Southern Dish",
    ingredientsList: [
        '2lbs Chicken Breast',
        '32 oz Oil (Canola or Peanut)',
        '1/4 Cu Creole Seasoning',
        '3 Cu Flour',
        '1 Cu Buttermilk',
        '1 teaspoon paprika',
        'salt and pepper to taste'
    ],
    directions: [
        'Take your cut up chicken pieces and skin them if you prefer.',
        'Put the flour in a large plastic bag (let the amount of chicken you are cooking dictate the amount of flour you use). Season the flour with paprika, salt and pepper to taste (paprika helps to brown the chicken).',
        'Place the coated chicken on a cookie sheet or tray, and cover with a clean dish towel or waxed paper. LET SIT UNTIL THE FLOUR IS OF A PASTE-LIKE CONSISTENCY. THIS IS CRUCIAL!',
        'Fill a large skillet (cast iron is best) about 1/3 to 1/2 full with vegetable oil. Heat until VERY hot.',
        'Put in as many chicken pieces as the skillet can hold. Brown the chicken in HOT oil on both sides.',
        'When browned, reduce heat and cover skillet; let cook for 30 minutes (the chicken will be cooked through but not crispy). Remove cover, raise heat again, and continue to fry until crispy.',
        'Drain the fried chicken on paper towels. Depending on how much chicken you have, you may have to fry in a few shifts. Keep the finished chicken in a slightly warm oven while preparing the res',
    ],
    totalTime: '40 mins',
    prepTime: '15 mins'
},
{
    _id: 'akljkljjjlk41234lkj',
    title: "Fried Chicken",
    subtitle: "Classic Southern Dish",
    ingredientsList: [
        '2lbs Chicken Breast',
        '32 oz Oil (Canola or Peanut)',
        '1/4 Cu Creole Seasoning',
        '3 Cu Flour',
        '1 Cu Buttermilk',
        '1 teaspoon paprika',
        'salt and pepper to taste'
    ],
    directions: [
        'Take your cut up chicken pieces and skin them if you prefer.',
        'Put the flour in a large plastic bag (let the amount of chicken you are cooking dictate the amount of flour you use). Season the flour with paprika, salt and pepper to taste (paprika helps to brown the chicken).',
        'Place the coated chicken on a cookie sheet or tray, and cover with a clean dish towel or waxed paper. LET SIT UNTIL THE FLOUR IS OF A PASTE-LIKE CONSISTENCY. THIS IS CRUCIAL!',
        'Fill a large skillet (cast iron is best) about 1/3 to 1/2 full with vegetable oil. Heat until VERY hot.',
        'Put in as many chicken pieces as the skillet can hold. Brown the chicken in HOT oil on both sides.',
        'When browned, reduce heat and cover skillet; let cook for 30 minutes (the chicken will be cooked through but not crispy). Remove cover, raise heat again, and continue to fry until crispy.',
        'Drain the fried chicken on paper towels. Depending on how much chicken you have, you may have to fry in a few shifts. Keep the finished chicken in a slightly warm oven while preparing the res',
    ],
    totalTime: '40 mins',
    prepTime: '15 mins'
},
{
    _id: 'asdf131sadfdfsafsdf4lkj',
    title: "Fried Chicken",
    subtitle: "Classic Southern Dish",
    ingredientsList: [
        '2lbs Chicken Breast',
        '32 oz Oil (Canola or Peanut)',
        '1/4 Cu Creole Seasoning',
        '3 Cu Flour',
        '1 Cu Buttermilk',
        '1 teaspoon paprika',
        'salt and pepper to taste'
    ],
    directions: [
        'Take your cut up chicken pieces and skin them if you prefer.',
        'Put the flour in a large plastic bag (let the amount of chicken you are cooking dictate the amount of flour you use). Season the flour with paprika, salt and pepper to taste (paprika helps to brown the chicken).',
        'Place the coated chicken on a cookie sheet or tray, and cover with a clean dish towel or waxed paper. LET SIT UNTIL THE FLOUR IS OF A PASTE-LIKE CONSISTENCY. THIS IS CRUCIAL!',
        'Fill a large skillet (cast iron is best) about 1/3 to 1/2 full with vegetable oil. Heat until VERY hot.',
        'Put in as many chicken pieces as the skillet can hold. Brown the chicken in HOT oil on both sides.',
        'When browned, reduce heat and cover skillet; let cook for 30 minutes (the chicken will be cooked through but not crispy). Remove cover, raise heat again, and continue to fry until crispy.',
        'Drain the fried chicken on paper towels. Depending on how much chicken you have, you may have to fry in a few shifts. Keep the finished chicken in a slightly warm oven while preparing the res',
    ],
    totalTime: '40 mins',
    prepTime: '15 mins'
},
{
    _id: 'adsfjlkm1324123432cdsd',
    title: "Fried Chicken",
    subtitle: "Classic Southern Dish",
    ingredientsList: [
        '2lbs Chicken Breast',
        '32 oz Oil (Canola or Peanut)',
        '1/4 Cu Creole Seasoning',
        '3 Cu Flour',
        '1 Cu Buttermilk',
        '1 teaspoon paprika',
        'salt and pepper to taste'
    ],
    directions: [
        'Take your cut up chicken pieces and skin them if you prefer.',
        'Put the flour in a large plastic bag (let the amount of chicken you are cooking dictate the amount of flour you use). Season the flour with paprika, salt and pepper to taste (paprika helps to brown the chicken).',
        'Place the coated chicken on a cookie sheet or tray, and cover with a clean dish towel or waxed paper. LET SIT UNTIL THE FLOUR IS OF A PASTE-LIKE CONSISTENCY. THIS IS CRUCIAL!',
        'Fill a large skillet (cast iron is best) about 1/3 to 1/2 full with vegetable oil. Heat until VERY hot.',
        'Put in as many chicken pieces as the skillet can hold. Brown the chicken in HOT oil on both sides.',
        'When browned, reduce heat and cover skillet; let cook for 30 minutes (the chicken will be cooked through but not crispy). Remove cover, raise heat again, and continue to fry until crispy.',
        'Drain the fried chicken on paper towels. Depending on how much chicken you have, you may have to fry in a few shifts. Keep the finished chicken in a slightly warm oven while preparing the res',
    ],
    totalTime: '40 mins',
    prepTime: '15 mins'
},
{
    title: "Fried Chicken",
    subtitle: "Classic Southern Dish",
    ingredients: [
        '2lbs Chicken Breast',
        '32 oz Oil (Canola or Peanut)',
        '1/4 Cu Creole Seasoning',
        '3 Cu Flour',
        '1 Cu Buttermilk',
        '1 teaspoon paprika',
        'salt and pepper to taste'
    ],
    directions: [
        'Take your cut up chicken pieces and skin them if you prefer.',
        'Put the flour in a large plastic bag (let the amount of chicken you are cooking dictate the amount of flour you use). Season the flour with paprika, salt and pepper to taste (paprika helps to brown the chicken).',
        'Place the coated chicken on a cookie sheet or tray, and cover with a clean dish towel or waxed paper. LET SIT UNTIL THE FLOUR IS OF A PASTE-LIKE CONSISTENCY. THIS IS CRUCIAL!',
        'Fill a large skillet (cast iron is best) about 1/3 to 1/2 full with vegetable oil. Heat until VERY hot.',
        'Put in as many chicken pieces as the skillet can hold. Brown the chicken in HOT oil on both sides.',
        'When browned, reduce heat and cover skillet; let cook for 30 minutes (the chicken will be cooked through but not crispy). Remove cover, raise heat again, and continue to fry until crispy.',
        'Drain the fried chicken on paper towels. Depending on how much chicken you have, you may have to fry in a few shifts. Keep the finished chicken in a slightly warm oven while preparing the res',
    ],
    totalTime: '40 mins',
    prepTime: '15 mins'
},

];

 module.exports =  testRecipe 